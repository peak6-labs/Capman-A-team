...
Measures whether stored `fair_prob` estimates were calibrated against realized
Kalshi settlement outcomes — the feedback loop the journal enables but does not
itself compute. It is strategy-agnostic: it scores whatever proposed a position
(research-agent ticket, three-leg, a manual order), so it survives changes to how
trades are sourced.
